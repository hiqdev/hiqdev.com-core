...
- .env - ENV=PROD, пароли можно сюда, а потом повторить их в params, я предпочитаю ложить параметры
  используемые другими (не PHP) технологиями используем
- composer.json - тут подключается основной проект и специфичные для него плагины
- src/
    - config/
        - params.php - явки, пароли, тайтл проекта, supportEmail
...
их можно и нужно создавать и назначать права системой сборки
(мы велосипедим свою, но это уже совсем другая история).
...
- отделяем переиспользуемый код от конкретной инсталяции с помощью "корня"
