...
радикальное решение - собирать конфиг за пределами приложения (хм, звучит просто
и очевидно, но, как известно, хорошая мысля приходит опосля).
...
Также порядок соблюдается в том что сначала обрабатываются все `defines`,
потом все `params`, а потом уже конфиги, таким образом константы и параметры
могут быть использованы в конфигах. В общем, процесс повторяет сборку конфигов
в `yii2-app-advanced`.
...
Эксплуатируем этот плагин довольно давно и успешно.
Также поддерживается `.env`, JSON и YAML.
...
организации больших переиспользуемых проектов. Повторю предлагаемое решение:
...
более внешний пакет использует внутренний,
в основном переиспользуя его поведение, но переопределяя свою специфику:
*"корень"* использует и уточняет основной проект,
основной проект - базовый, базовый проект - фреймворк.
В принципе, уровней может больше, моё мнение что не меньше:
*"корень"*, проект и базовый проект - выкинув или объединив что-то можно только потерять,
а не выиграть.
...
и настройке под данную инсталяцию проекта. Да-да, можно и желательно, обойтись
всего тремя файлами:

- `.env` - переменные окружения, например,`ENV=prod`
- `composer.json` - тут подключается основной проект и специфичные для него плагины
- `src/config/params.php` - явки, пароли, тайтл проекта, cookieValidationKey, supportEmail, ...
...
Учитывая общеупотребимость `.env` лучшими претендентами на вынос в `.env` являются
...
-- Эээ... А можно ещё конкретнее? Покажите мне код!
...
Используя этот подход сделан [Asset Packagist](https://asset-packagist.org) -
...
- *"корень"* - [hiqdev/asset-packagist.dev](https://github.com/hiqdev/asset-packagist.dev)
    ...
    - проект - [hiqdev/asset-packagist](https://github.com/hiqdev/asset-packagist.org)
...
В README корня описано как поднять проект у себя.
В `composer.json` корня можно поменять `yii2-theme-original` на `yii2-theme-flat`
запустить `composer update` и сайт переоденется в новую тему. Так просто.
...
- переиспользуем код вместе с конфигурацией
...
волосы стали мягкие и шелковистые: разделяем и властвуем, клепаем плагины как
пирожки, [100+] и останавливаться не собираемся, новая задача - новый плагин.
